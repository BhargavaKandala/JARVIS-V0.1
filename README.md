...
JARVIS-OS is modeled on the decoupled Fable 5 OS framework [source: 1]. It handles multi-threaded local speech processing, cross-functional dynamic skill mapping, and persistent asynchronous long-term memory via local Markdown graphs.

---

## ⚡ Core System Architecture

1. **The Brain (Skill Paradigm):** Orchestrated by Gemini 2.5 Flash. Native system scripts and dynamic skill subdirectories are evaluated, mapped, and appended to the LLM's functional execution scope at runtime.
2. **The Memory (Obsidian Vault RAG):** Bypasses centralized relational databases. System events, commands, and structured reports are written as standalone `.md` files via local secure HTTPS REST channels.
3. **The Voice (100% Offline Pipe):**
...
4. **The Face (V.A.U.L.T. HUD):** A dedicated multi-threaded GUI built with `CustomTkinter`. Audio acquisition, orchestration processing, and text streaming execute on decoupled background threads to prevent main interface freezing.
...
Verify that kokoro-v1.0.onnx and voices-v1.0.bin are present in your workspace root.

4. Mounting the Obsidian Brain
Initialize a clean local vault inside your Obsidian desktop app.

Navigate to Settings -> Community Plugins and toggle "Restricted Mode" off.

Install, activate, and enable the Local REST API plugin.

Open the plugin configurations: Verify that the endpoint utilizes secure HTTPS (https://127.0.0.1:27123) and copy your unique alphanumeric API Key.
...
OBSIDIAN_URL=[https://127.0.0.1:27123](https://127.0.0.1:27123)
OBSIDIAN_API_KEY=your_copied_raw_api_key_here
Crucial: Ensure no extra whitespaces, quotations, or strings like "Bearer" prefix the Obsidian token.
